...
func main() {
	...
	for idx := 0; idx < len(matrix); idx++ {
		for jdx := 0; jdx < len(matrix[0]); jdx++ {
			if matrix[idx][jdx] == 0 {
				allStarts = append(allStarts, []int{idx, jdx})
			}
		}
	}

	bestSofar := Solve(matrix, start, end)
	for _, beginning := range allStarts {
		current := Solve(matrix, beginning, end)
		if current < bestSofar && current > 0 {
			bestSofar = current
		}
	}
